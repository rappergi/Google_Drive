...
def serviceaccountfactory(
    ...
    if create_projects:
        ...
        if create_projects > 0:
            ...
            if current_count + create_projects <= max_projects:
                ...
                sys.exit('Không, bạn không thể tạo %d new project (s).\n'
                      'Vui lòng giảm giá trị của --quick-setup.\n'
                      'Hãy nhớ rằng bạn hoàn toàn có thể tạo %d projects (%d already).\n'
                      'Vui lòng không xóa các dự án hiện có trừ khi bạn biết mình đang làm gì' % (create_projects, max_projects, current_count))
        else:
            print('Ghi đè tất cả các Services Accounts trong các dự án hiện có.\n'
                  'Vì vậy, hãy chắc chắn rằng bạn đã có một số dự án.')
    ...
    if delete_sas:
        ...
        if delete_sas == '~':
            ...
            std = _get_projects(cloud)
        for i in std:
            print('Xóa Service Accounts trong %s' % i)
            _delete_sas(iam,i)
